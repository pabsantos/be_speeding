...
make_lisa_maps <- function(lisa_taz, var) {
  ...
  cluster_colors <- c("#eeeeee", "#FF0000", "#0000FF", "#a7adf9", "#f4ada8", 
                      "#464646", "#999999")
  
  # Plot
  p1 <- ggplot() +
    geom_sf(data = taz, fill = "Grey70", color = "Grey50", lwd = 0.1) +
    geom_sf(data = lisa_taz, aes(fill = cluster), color = "Grey20", 
            lwd = 0.1) +
    theme_void() +
    scale_fill_manual(values = cluster_colors) +
    labs(fill = label) +
    theme(legend.position = c(0.99, 0.21),
...
all_lisa_maps <- list(lisa_coefs_maps, lisa_sp_maps, 
                      lisa_lm_sp_maps, lisa_r2_maps)
...
lisa_misc_cluster <- map(list(lisa_sp, lisa_lm_sp, lisa_r2), 
                         make_cluster_data)
